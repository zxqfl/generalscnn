...
Map.prototype.indexToPoint = function(index) {
	return [
...
Map.prototype.indexOwnedBy = function(index, player) {
	return this.tileAt(index) == player;
};

Map.prototype.pointOwnedBy = function(row, col, player) {
	if (row < 0)
		return false;
	if (col < 0)
		return false;
	...
	if (col >= this.width)
		return false;
	return this.indexOwnedBy(this.indexFrom(row, col), player);
};

Map.prototype.pointVisibleTo = function(row, col, player) {
	return false ||
		this.pointOwnedBy(row+0, col+0, player) ||
		this.pointOwnedBy(row-1, col-1, player) ||
		this.pointOwnedBy(row-1, col+0, player) ||
		this.pointOwnedBy(row-1, col+1, player) ||
		this.pointOwnedBy(row+0, col+1, player) ||
		this.pointOwnedBy(row+1, col+1, player) ||
		this.pointOwnedBy(row+1, col+0, player) ||
		this.pointOwnedBy(row+1, col-1, player) ||
		this.pointOwnedBy(row+0, col-1, player);
...
Map.prototype.indexVisibleTo = function(index, player) {
	var pt = this.indexToPoint(index);
	return this.pointVisibleTo(pt[0], pt[1], player);
}
